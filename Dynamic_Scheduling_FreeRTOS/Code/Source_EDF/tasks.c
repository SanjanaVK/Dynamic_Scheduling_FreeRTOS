...
/*Description : Rearrange function and its calls for priority assignment
...
//double util=0.0;
//double fmax=50000000.0;
//double fest=50000000.0;
...
portBASE_TYPE xTaskGenericCreate( TaskFunction_t  pxTaskCode, const char * const pcName, const uint16_t usStackDepth, void  * const pvParameters, portBASE_TYPE uxPriority, TaskHandle_t * const pxCreatedTask, StackType_t * const puxStackBuffer, const MemoryRegion_t * const xRegions )
{
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE xAlreadyYielded, xShouldDelay = pdFALSE;
        ...
        vTaskSuspendAll();
        {
            ...
            //const double xConstTickCount = actualTime;
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE uxTaskPriorityGet( TaskHandle_t xTask )
    {
    tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
    ...
    void vTaskPrioritySet( xTaskHandle xTask, portBASE_TYPE uxNewPriority )
    {
    tskTCB *pxTCB;
        ...
        uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR();
        {
            if( xTaskIsTaskSuspended( pxTCB ) == pdTRUE )
            {
                ...
                if( uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE )
                {
                    ...
                    if( pxTCB->period + pxTCB->waketime >= pxCurrentTCB->period + pxCurrentTCB->waketime )
                    {
                        xYieldRequired = pdTRUE;
                    }
...
portBASE_TYPE xReturn;
    ...
    #if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
    {
        ...
        xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL,  tskIDLE_PRIORITY , &xIdleTaskHandle ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
        tskTCB *idletask = prvGetTCBFromHandle (xIdleTaskHandle);
        idletask->period=0xffff;
        idletask->waketime=0;
    }
    #else
    {
        ...
        if( uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE )
        {
            if( uxCurrentNumberOfTasks > ( unsigned portBASE_TYPE ) 0U )
            {
                ...
                while( listLIST_IS_EMPTY( &xPendingReadyList ) == pdFALSE )
                {
                    ...
                    if( (pxTCB->waketime+pxTCB->period) <= (pxCurrentTCB->waketime+pxCurrentTCB->period) )
                    {
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE uxTaskGetSystemState( TaskStatus_t * const pxTaskStatusArray, const portBASE_TYPE uxArraySize, uint32_t * const pulTotalRunTime )
    {
...
portBASE_TYPE xSwitchRequired = pdFALSE;
    ...
    if( uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE )
    {
        ...
        ++xTickCount;
        //actualTime=actualTime+(fmax/fest);
        //pxCurrentTCB->AET=pxCurrentTCB->AET+(fmax/fest);
        {
            /* Minor optimisation.  The tick count cannot change in this
            block. */
            //const double xConstTickCount = actualTime;
            ...
            {
                //UARTprintf("condition satisfied");
                for( ;; )
                {
                    if( listLIST_IS_EMPTY( pxDelayedTaskList ) != pdFALSE )
                    {
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE uxTaskGetTaskNumber( TaskHandle_t xTask )
    {
    unsigned portBASE_TYPE uxReturn;
    tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
    ...
    void vTaskSetTaskNumber( xTaskHandle xTask, const portBASE_TYPE uxHandle )
    {
    tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE uxTaskGetStackHighWaterMark( TaskHandle_t xTask )
    {
    tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
    ...
    portBASE_TYPE xTaskPriorityDisinherit( xTaskHandle const pxMutexHolder )
    {
    tskTCB * const pxTCB = ( tskTCB * ) pxMutexHolder;
